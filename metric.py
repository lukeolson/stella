        ...
        if axis == 0:
            ...
            for j in range(1, x.shape[0] - 1):
                for i in range(1, x.shape[1]-1, 2):
                    jj = j-1
                    if i < x.shape[1] - 1:
                        f0 = interp1d(np.arange(3), x[j-1, i-1:i+2],kind='linear')
                        f1 = interp1d(np.arange(3), x[j+1, i-1:i+2],kind='linear')
                        XHx_t[jj, i-1] = (f1(.5) - f0(.5)) * .5
                        XHx_t[jj, i] = (f1(1.5) - f0(1.5)) * .5
                        f0 = interp1d(np.arange(3), y[j-1, i-1:i+2],kind='linear')
                        f1 = interp1d(np.arange(3), y[j+1, i-1:i+2],kind='linear')
                        XHy_t[jj, i-1] = (f1(.5) - f0(.5)) * .5
                        XHy_t[jj, i] = (f1(1.5) - f0(1.5)) * .5
                    elif i == x.shape[1] - 1:
                        f0 = interp1d(np.arange(3), x[j-1, i-2:i+1],kind='linear')
                        f1 = interp1d(np.arange(3), x[j+1, i-2:i+1],kind='linear')
                        XHx_t[jj, i-1] = (f1(.5) - f0(.5)) * .5
                        f0 = interp1d(np.arange(3), y[j-1, i-2:i+1],kind='linear')
                        f1 = interp1d(np.arange(3), y[j+1, i-2:i+1],kind='linear')
                        XHy_t[jj, i-1] = (f1(.5) - f0(.5)) * .5
